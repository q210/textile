 ...
 *     - Allows to set custom formatting escape sequences - textile formatting will be disabled for a substring, enveloped in these tokens.
 *     - _default_ is `['==']`
 *
 * ## History
 *
...
(function (root, name, factory) {
  ...
  function TextileConverter (src, options) {
    ...
    this.options.escapeSequences = options && options.escapeSequences !== undefined ? options.escapeSequences : ['=='];
    this.out = '';
    this.footnotes = [];
  }
  ...
  TextileConverter.prototype = {
    ...
    replaceCustomEscapeSeqs: function (s) {
      // envelop every substring for s limited by any of symbol sequences from `escapeSequences` option
      // with <notextile> tag

      // replace string limited by any of strings from `escapeSequences` with <notextile>
      for (i = 0; i < this.options.escapeSequences.length; i++) {
        var escSeq = this.regexpEscape(this.options.escapeSequences[i]);
        var escapeSeqReplacer =  new RegExp(escSeq + "((?:.|\n)*?)" + escSeq, "g");

        s = s.replace(escapeSeqReplacer, "<notextile>$1</notextile>");
      }
      return s
    },
    ...
    convert: function () {
      ...
      this.src = this.replaceCustomEscapeSeqs(this.src);
