...
(function (root, name, factory) {
  ...
  return function () {
    function TextileConverter (src, options) {
      ...
      this.options = {};
      this.options.wrapBlocks = options.wrapBlocks != undefined ? options.wrapBlocks : true;
      this.out = '';
      this.footnotes = [];
    }
    ...
    TextileConverter.prototype = {
      ...
      footnoteID: function (n) {
        if (!this.footnotes[n])
          this.footnotes[n] = 'fn' + (Math.random() + '').slice(2) + (+new Date);
    ...
    var re = {
      ...
      footnote: new RegExp('^fn(\\d+)' + re.attr.expr.source + '?\\.(\\.)? '),
      pre: new RegExp('^pre' + re.attr.expr.source + '?\.(\.)? '),
      raw: new RegExp('^notextile' + re.attr.expr.source + '?\.(\.)? '),

      html: {
        pre: /^<pre((?:\s+\w+(?:\s*=\s*(?:".*?"|'.*?'|[^'">\s]+))?)+\s*|\s*)>((?:.|\n(?!\n))*)<\/pre>(?:\n\n|$)/,
        source: /^<(\w+)((\s+\w+(\s*=\s*(?:".*?"|'.*?'|[^'">\s]+))?)+\s*|\s*)>(.|\n(?!\n))*<\/\1>(\n\n|$)/
      }
    };
    ...
    function getAttributes (attr) {
      ...
      var opt = {
        style: ''
      }, paddingLeft = 0, paddingRight = 0, m, style, n;
    ...
    var list = {
      ...
      run: function () {
        var m, listType = '', tags = [], line, attr, level = 0, tag, listType = '', o;
    ...
    var footnote = {
      match: function () { return re.footnote.test(this.src); },
      run: function () {
    ...
    var blockcode = {
      ...
      run: function () {
        ...
        carryon = m[2]!=undefined;
        this.src = this.src.slice(m[0].length);
        block = this.escapeHTML(this.getBlock());
    ...
    var preBlock = {
      ...
      run: function () {
        var m, attr, tag, tag, carryon, block;
    ...
    var normalBlock = {
      ...
      run: function () {
        var n, tagName, attr, tag, carryon, block;
    ...
    var preHTMLBlock = {
      ...
      run: function () {
        var m = re.html.pre.exec(this.src);
        this.src = this.src.slice(m[0].length);
        var attr = m[1];
